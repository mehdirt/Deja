...
**Proof points (all true, all verifiable):** zero network calls (open source after Week 3 — auditable) · PII redacted before storage · credential fields never read · pause/per-site controls · JSON/Markdown export, no lock-in · $0, no account.
...
| MIT LICENSE + uncomment site source links + repo-public prep | Claude | — | Wk 3 |
...
1. Repo → public (LICENSE, links uncommented); listing → **Public**; swap `REPLACE_EXTENSION_ID`; redeploy site (1h)
