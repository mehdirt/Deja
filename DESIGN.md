...
- **Overlays use the system UI font**, not Figtree. An overlay sitting on chatgpt.com should read as part of that page, not as a foreign widget. Mono is only for `{blank}` / `[email_1]` tokens in previews, hostnames, and the literal `//` hint — never for human field labels.
- **Cards match Library / landing**, denser: raised `--dj-surface`, **16px** radius (same as library cards), soft pop shadow, accent-tinted border on row hover. Rows are miniature PromptCards (platform chip + body + meta). Buttons use the 11px btn radius and CTA shadow on primary.
- **The panel (dot click)** mirrors the popup: brand header (`deja` wordmark + subtitle), rounded search well with icon, card stack on paper, short footer actions.
- **Enter motion matches the extension pages**: `.2s` with `cubic-bezier(0.16, 1, 0.3, 1)`.
- **Two extra tokens exist only here.** `--dj-accent-text` is the accent at text weight — dark mode needs the lighter step to stay legible on a dark card. `--dj-shadow-sm` / `--dj-shadow-cta` mirror landing/library elevation.
...
- **Anchoring is shared too** (`src/content/shared/anchor.ts`): clamp to the viewport with an 8px margin, flip above↔below when there's no room, throttle scroll/resize at 100ms.
- **The ambient dot sits *inside* the message box**, hugging a corner, **26px rounded square** (matches the mark tile, not a floating orb) with an 8px inset. Inside is what makes it read as part of the box rather than as something floating near it — and the selectors resolve to the editable itself, so "outside the right edge" is the site's own chrome, which differs on every site and looks homeless. The corner isn't fixed: `pickSpot` tries bottom-right, top-right, top-left, then outside, hit-testing each with `elementFromPoint` and skipping any square the site already uses for a control. A site can pin a corner from its own content script (`dotCorner`), which is still collision-checked. Its count badge leans *into* the box (the shoulder away from the nearest edge), never out over it.
...
| PII vault / deanonymize map | remember for fill-in |
...
| select several / bulk select | select a few |
