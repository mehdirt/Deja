...
- **Overlays use the system UI font**, not Figtree, for body UI. An overlay sitting on chatgpt.com should read as part of that page. **Exception:** the lowercase `deja` wordmark always uses bundled **Literata** (loaded into the shadow tree via `chrome.runtime.getURL('fonts/…')`). Mono is only for `{blank}` / `[email_1]` tokens in previews, hostnames, and the literal `//` hint — never for human field labels.
...
- **The ambient dot sits left of the end action** (Send / Voice / mic) on the composer **shell**, centred on that control — **26px**, ~6–8px gap. Skips model chips (Sonnet/Flash/Fast). Shell prefers a tight pill — not a full-bleed `<form>` (Grok). Fallback: shell bottom-right.
  - **Status (2026-08-11): deactivated.** `PRESENCE_ENABLED = false` in `presence.ts` — mark + its panel are off until cross-site placement is finished. Capture, resurface tooltip, and `//` picker stay on.
