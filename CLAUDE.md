...
- `db.ts` — Dexie schema (single `prompts` table) and all CRUD, including JSON-import row normalization (`normalizeImportedRow`, tested in `import.test.ts`). Library delete is soft for a short Undo window (`deletedAt`), then hard-deleted; a wake/open sweep clears leftover tombstones. Hard-delete also used for undo-capture, library-cap trim, and "clear all".
...
- `classify.ts` — selective-capture classifier: skips storing throwaway prompts by strength (`off`/`balanced`/`strict`). Pure; runs at capture time in the background worker. `hasSubstance()` judges value in **everyday** terms as well as technical ones — a specified tone, audience, format, length, or a real question rescues a short prompt, exactly as a code fence does. Don't let this drift back to code-only signals. `balanced` recognises a throwaway three ways, and the third is the only one that generalizes: `TRIVIAL` (exact phrases) and the all-filler rule are closed sets that skip only what someone thought to list, while `isTopicless()` asks whether the message names a subject at all — which is what catches "hi there", "quick question" and "i need help" without an entry each.

Rules that are load-bearing and easy to undo by accident:

- A word belongs in `FILLER` only if it is meaningless alone **and** can't be the backbone of a clause (listing the interrogatives there once made "what is this" and "how do i do this" vanish). `SOCIAL` and `GENERIC` — the two sets `isTopicless` adds — are held to a *stricter* bar still: interrogatives, the copula and modals must never appear in them, or the same small-word questions vanish again. A word that is glue only when sent bare belongs in `TRIVIAL`.
- All three sets are English, so non-English text always reads as subject-bearing and is kept. That's the correct failure direction and it is pinned by a test — don't "fix" it with a script-blind heuristic. The obvious one ("a word with no vowel is keyboard mashing") is wrong twice: `asdf` has a vowel, and Persian/Arabic have no written ones. `REPEATED_UNIT` exists because it needs no vowel notion.
- The `reason` a prompt was skipped travels out in `CaptureResponse` because the page treats the three differently: `trivial` glue stays quiet, while `short` (from `strict`) and `vague` (from `balanced`) are judgment calls and get offered back with a *Keep it* button that hand-saves through `SAVE_MANUAL`. A skip is permanent, so any rule that makes a judgment call has to offer the call back — and `vague` needs it *most*, being the one that fires on the default strength.
- `strict`'s gate is deliberately blunt: short unspecified asks like "ideas for date night" are meant to go, with *Keep it* as the recourse. Length is one of three qualifying routes and removing it deletes real prompts — a long everyday question made of small words ("what should i say to my boss about working from home") clears no other bar.
