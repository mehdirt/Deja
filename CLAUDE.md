...
- `pii.ts` — local PII detection + redaction (regex + Luhn/SSN/IBAN checks, known secret shapes). Replaces detected personal info with numbered labels (`[email_1]`, `[card_1]`, …) BEFORE storage. Optional `piiVault.ts` remembers originals for Fill-in only. On by default (per-category togglable in prefs). Pure; runs first in the background `PROMPT_CAPTURED` handler and also on the resurface query so both sides match.
